...
const Map = ({ ranger, arrondissements }) => {
  ...
  const updateMap = () => {
    ...
    console.error(arrondissements)

    for (const number in arrondissements) {
      const elementArrondissements = mapSvg.getElementsByClassName(`${number}_arrondissement`)[0]

      let i = 3
      let idClass = 3
      do {
        console.log(arrondissements[number].length, ranger.result[i])
        idClass = i
        i--
      } while (arrondissements[number].length <= ranger.result[i].end && i != 0)
      elementArrondissements.classList.add(colorClass[idClass])
    }
  }
  ...
  return (
    <div>
      <svg xmlns="http://www.w3.org/2000/svg" width="577.645" height="391.181" viewBox="0 0 577.645 391.181">
        <g ref={(el) => { setMapSvg(el) }} className="map__svg" data-name="map-paris 1 3" transform="translate(-3.171 -4.578)">
